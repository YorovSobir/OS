...
static void balloc_parse_mmap(const struct mboot_info *info)
{
	...
	__balloc_add_range(&memory_map, fs_addr.start, fs_addr.end);
	__balloc_add_range(&free_ranges, fs_addr.start, fs_addr.end);
	...
	__balloc_remove_range(&free_ranges, fs_addr.start, fs_addr.end);
